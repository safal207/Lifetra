...
use crate::{
    ActionId, AuthorityTicket, ExecutionMode, IdempotencyBinding, PostgresTransactionRetryPolicy,
    PostgresUnifiedFencedRuntime, PostgresUnifiedFencedStore, RecoveryWorkerId,
    UnifiedActionRecord, UnifiedFencedStore, UnifiedLeaseAuthority,
};
    ...
    loop {
        ...
        if mode == ProxyFaultMode::DropCommitAckOnce
            ...
            && fault_for_server_to_client
                ...
                .is_ok()
        {
            // Reading the server response proves PostgreSQL reached the post-COMMIT
            // response boundary. Drop that response instead of forwarding it, so
            // the client sees an acknowledgement loss while durable state remains.
